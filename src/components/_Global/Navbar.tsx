...
const Navbar: React.FC = () => {
    ...
    return (
        <div className="flex flex-row md:gap-3 gap-1 absolute top-0 right-0 z-10 md:p-6 p-3">
...
const NavLink: React.FC<NavLinkProps> = ({ href, icon }) => {
    ...
    return (
        <Link href={href}>
            <FontAwesomeIcon
                className={`${isActive ? 'text-content ' : 'text-subtitle '} md:text-3xl text-xl m-3`}
                icon={icon}
            />
        </Link>
    )
